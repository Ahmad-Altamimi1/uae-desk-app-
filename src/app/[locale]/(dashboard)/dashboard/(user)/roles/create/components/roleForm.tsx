...
export default function RoleCreateForm({ permissionsList }: { permissionsList: Record<string, IResponseUsersPermissions[]> }) {
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="mt-8">
                    <InputCollectionLabel
                        ...
                        className="mb-6"
                    />                    <div className="mb-4">
                        <CustomCheckbox name="all_permissions" label="Select All Permissions" control={control} />
                    </div>
                    <hr />

                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-5">
                        {Object.entries(permissionsList).map(([section, permissions], parentIndex) => (
                            <div key={section}>
                                <h4 className="text-md font-semibold text-green-800 mb-2">{section}</h4>
                                <CustomCheckbox name={`all_${section.toLowerCase()}`} label={`All ${section} Permissions`} control={control} />
